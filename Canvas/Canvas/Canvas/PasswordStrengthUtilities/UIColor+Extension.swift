...
public extension UIColor {
    ...
    class func colorFrom(hexString hexStr: String, alpha: CGFloat = 1) -> UIColor? {
        ...
        //strlen replaced to characStringCount
        let characStringCount = cStr.count
        if (characStringCount == 3 || characStringCount == 4) {
            // Make it double
            var str2 = ""
            for ch in cStr {
                str2 += "\(ch)\(ch)"
            ...
            cStr = str2
        } else if (characStringCount == 6 || characStringCount == 8) {
            // Do nothing
        } else {
            return nil
        }
