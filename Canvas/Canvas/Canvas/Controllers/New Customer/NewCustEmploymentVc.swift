...
class NewCustEmploymentVc: UIViewController, UITextFieldDelegate, UIDropDownrDelegateM, EasyTipViewDelegate {
    ...
    func assignLabels() {
        ...
        if Global.shared.new_employerName_is_required == true {
            employeNameLbl.text = Global.shared.employNameTxt + "*"
        }
        else {
            employeNameLbl.text = Global.shared.employNameTxt
        }
        if Global.shared.new_employerID_is_required == true {
            employIdLbl.text = Global.shared.employeIdTxt + "*"
        }
        else {
            employIdLbl.text = Global.shared.employeIdTxt
        }
        if Global.shared.new_employerLocation_is_required == true {
            employLocatnLbl.text = Global.shared.employLoctn + "*"
        }
        else {
            employLocatnLbl.text = Global.shared.employLoctn
        }
        if Global.shared.new_employerDesigntn_is_required == true {
        ...
        employField.maxLength = Global.shared.new_employerName_max_length
        employFieldLbl.maxLength = Global.shared.new_employerID_max_length
        employLocatnField.maxLength = Global.shared.new_employerLocation_max_length
        designtnAnswerLbl.maxLength = Global.shared.new_employerDesigntn_max_length
        salaryField.maxLength = Global.shared.new_salary_max_length
        otherIncomeField.maxLength = Global.shared.new_otherIncome_max_length
        
        employField.valueType = Global.shared.getFieldType(text: Global.shared.new_employerName_field_accept)
        employFieldLbl.valueType = Global.shared.getFieldType(text: Global.shared.new_employerID_field_accept)
        employLocatnField.valueType = Global.shared.getFieldType(text: Global.shared.new_employerLocation_field_accept)
        designtnAnswerLbl.valueType = Global.shared.getFieldType(text: Global.shared.new_employerDesigntn_field_accept)
     //   salaryField.valueType = Global.shared.getFieldType(text: Global.shared.new_salary_field_accept)
    //    otherIncomeField.valueType = Global.shared.getFieldType(text: Global.shared.new_otherIncome_field_accept)
    }
    ...
    @IBAction func nextBtnActn(_ sender: Any) {
        ...
        if Global.shared.new_employerName_is_required == true && employField.text == "" {
            employField.becomeFirstResponder()
           employNameReqLbl.isHidden = false
           
       }
        else if Global.shared.new_employerLocation_is_required == true && employLocatnField.text == "" {
            employLocatnField.becomeFirstResponder()
            employLoctnReqLbl.isHidden = false
        }
        else if Global.shared.new_occupation_is_required == true && occuptnAnswerLbl.text == "" {
            occuptnAnswerLbl.becomeFirstResponder()
            occuptnReqLbl.isHidden = false
            
        }
       
        else   if Global.shared.new_employerID_is_required == true && employFieldLbl.text == ""  {
            employFieldLbl.becomeFirstResponder()
            employIdReqLbl.isHidden = false
            
        }
       
        else   if Global.shared.new_employerDesigntn_is_required == true && designtnAnswerLbl.text == "" {
            designtnAnswerLbl.becomeFirstResponder()
            designtnReqLbl.isHidden = false
            
        }
        else   if Global.shared.new_salary_is_required == true && salaryField.text == "" {
            salaryField.becomeFirstResponder()
            salaryReqLbl.isHidden = false
            
        }
        else   if Global.shared.new_otherIncome_is_required == true &&  otherIncomeField.text == ""{
            ...
            otherIncomeReqLbl.isHidden = false
                ...
                // NewUser.shared.occupation = occuptnAnswerLbl.text ?? ""
          
            NewUser.shared.employerName = employField.text ?? ""
            NewUser.shared.employeeID = employFieldLbl.text ?? ""
            NewUser.shared.employLocation = employLocatnField.text ?? ""
            NewUser.shared.designation = designtnAnswerLbl.text ?? ""
            NewUser.shared.salary = salaryField.text ?? ""
            NewUser.shared.otherIncome = otherIncomeField.text ?? ""
            self.pushViewController(controller: NewCustBankVc.initiateController(),isGestureEnable: false, animated: false)
        }
    ...
    @IBAction func occuptnDrpDwnActn(_ sender: Any) {
     /*   let pickerData = Global.shared.pickerResponseOccupation
        PickerDialog().show(title: "", options: pickerData , selected: "") {
            ...
            NewUser.shared.occupation = Global.shared.occupationValues[indexS ?? 0]
        }*/
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        ...
        else if textField == occuptnAnswerLbl {
           // occuptnAnswerLbl
            if occuptnAnswerLbl.text!.count ==  0 {
                occuptnReqLbl.isHidden = false
            }
            else {
                occuptnReqLbl.isHidden = true
            }
        }
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if textField == occuptnAnswerLbl {
          
                if occuptnAnswerLbl.text!.count <= -1 {
                    occuptnReqLbl.isHidden = false
                }
                else {
                    occuptnReqLbl.isHidden = true
    ...
    @IBAction func otherIncomeToolTipActn(_ sender: Any) {
        ...
        if self.easyTipView == nil
        {
            self.easyTipView = EasyTipView(text: Global.shared.otherIncomeToolTip)
            self.easyTipView.show(animated: true, forView: sender as! UIView, withinSuperview: nil)
        }
        else
        {
